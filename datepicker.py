...
class DatePicker(UserControl):
    ...
    def __init__(self, 
            ...
            selected_date: List[datetime] = None,  # Modificação na anotação de tipo
            selection_type: Union[SelectionType, int] = SelectionType.SINGLE,
            disable_to: datetime = None, 
            disable_from: datetime = None,
            holidays: List[datetime] = None,  # Modificação na anotação de tipo
    ...
    def _create_calendar(self, year, month, hour, minute, hide_ymhm = False):
        ...
        for w in range(0, weeks_rows_num):
            ...
            for d in days[w]:
                ...
                if self.hide_prev_next_month_days and not is_main_month:
                    row.append(Text("", width=self.CELL_SIZE, height=self.CELL_SIZE,))
                    continue
                ...
                if self.disable_from and self._reset_time(d) > self._reset_time(self.disable_from):
                    is_day_disabled = True
                
                if self.disable_to and self._reset_time(d) < self._reset_time(self.disable_to):
                    is_day_disabled = True
        ...
        week_rows_controls.append(Column(week_rows_days_controls, alignment=MainAxisAlignment.START, spacing=0))
    ...
    def _year_month_selectors(self, year, month, hide_ymhm = False):
        prev_year = IconButton(icon=icons.ARROW_BACK, data=self.PREV_YEAR, on_click=self._adjust_calendar) if not hide_ymhm else Text(self.EMPTY, height=self.CELL_SIZE,)
        next_year = IconButton(icon=icons.ARROW_FORWARD, data=self.NEXT_YEAR, on_click=self._adjust_calendar) if not hide_ymhm else Text(self.EMPTY)
        prev_month = IconButton(icon=icons.ARROW_BACK, data=self.PREV_MONTH, on_click=self._adjust_calendar) if not hide_ymhm else Text(self.EMPTY)
        next_month = IconButton(icon=icons.ARROW_FORWARD, data=self.NEXT_MONTH, on_click=self._adjust_calendar) if not hide_ymhm else Text(self.EMPTY)
        ym = Row([
                    Row([
                        prev_year,
                        Text(year),
    ...
    def _hour_minute_selector(self, hour, minute):
        hm = Row(
            [
                Row([
                    IconButton(icon=icons.ARROW_BACK, data=self.PREV_HOUR, on_click=self._adjust_hh_min),
                    Text(hour),
                    IconButton(icon=icons.ARROW_FORWARD, data=self.NEXT_HOUR, on_click=self._adjust_hh_min),
                ]),
                Text(":"),
                Row([
                    IconButton(icon=icons.ARROW_BACK, data=self.PREV_MINUTE, on_click=self._adjust_hh_min),
                    Text(minute),
                    IconButton(icon=icons.ARROW_FORWARD, data=self.NEXT_MINUTE, on_click=self._adjust_hh_min),
                ]),
            ], spacing=48, alignment=MainAxisAlignment.SPACE_EVENLY)
    ...
    def _create_layout(self, year, month, hour, minute):
        ...
        if self.show_three_months:
            ...
            rows.append(Column(week_rows_controls_prev, width=self.LAYOUT_WIDTH, spacing=10))
        ...
        if self.show_three_months:
            ...
            rows.append(Column(week_rows_controls_next, width=self.LAYOUT_WIDTH, spacing=10))
    ...
    def _select_date(self, e: ControlEvent):
        ...
        result: datetime.strftime('%d/%m/%Y') = e.control.data
    ...
    def _update_calendar(self):
        self.cal_container.content = Row(self._create_layout(self.yy, self.mm, self.hour, self.minute))
        cal_height = self._calculate_heigth(self.yy, self.mm)
